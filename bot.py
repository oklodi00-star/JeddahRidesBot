"""
🤖 بوت المشاوير الذكي المتكامل
جميع المميزات - بدون إشعارات خاصة - يتذكر الأدوار نهائياً
"""
...
NORMAL_TRIP_WORDS = [
    ...
    "اوصلني", "ودني", "عايز مشوار", "محتاج توصيل"
]
...
class NLP:
    ...
    def extract_date_time(self, text):
        ...
        if 'بكرة' in text or 'غدا' in text:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for bad_word in BAD_WORDS:
        if nlp.normalize(bad_word) in normalized:
            await update.message.reply_text("⚠️ من فضلك التزم بالأدب في الكلام")
            return
...
"""
    ...
    if user_role:
        await update.message.reply_text(
            ...
            "• اطلب مشوار: «عايز مشوار من X إلى Y»\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "reg_driver":
        ...
        await query.edit_message_text(
            ...
            f"لن نسألك مرة أخرى عن صفتك.\n\n"
            f"الآن يمكنك:\n"
            f"• طلب مشوار بقول «عايز مشوار من... إلى...»",
            parse_mode=ParseMode.HTML,
            reply_markup=main_menu(user_id)
        )
        ...
        await query.edit_message_text(
            ...
            "مثال: «عايز مشوار من الحمراء للسلامة بكرة 8 الصبح»\n\n"
            "للمشوار الشهري: «محتاج كابتن شهري للدوام»",
            parse_mode=ParseMode.HTML
        )

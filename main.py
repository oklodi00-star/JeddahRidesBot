...
def init_db():
    with db() as con:
        ...
        # ----------------------------------------------------
        # التأكد من أعمدة قاعدة البيانات القديمة
        # ----------------------------------------------------
...
def save_user(user):
    ...
    with db() as con:
        ...
        # مهم:
        # لا نغير is_driver أو is_customer هنا.
        # الصفة المسجلة تبقى محفوظة حتى بعد تحديث الاسم/اليوزر.
        cur.execute("""
            INSERT INTO users (
                user_id,
                name,
...
def display_user(user):
    ...
    badge = get_role_badge(user.id)

    if badge:
        return f"<b>{html(badge)} {name}</b>"
...
async def member_left_handler(update, context):
    ...
    badge = get_role_badge(left_member.id)
...
async def handle_location(update, context):
    ...
    # ========================================================
    # أهم تعديل:
    # إذا كان مسجلًا ككابتن، لا نسأله أبدًا أن يسجل نفسه.
    # ========================================================

    driver_registered = is_driver(user.id)

    # ========================================================
    # التعرف التلقائي فقط على عبارات التواجد الصريحة للكابتن
    # ========================================================
    ...
    # ========================================================
    # إذا لم يكن مسجلًا ولا يوجد دليل واضح أنه كابتن
    # ========================================================
...
def looks_like_trip(text):
    ...
    # ========================================================
    # مهم:
    # إعلان موقع الكابتن لا يصبح طلب عميل.
    # ========================================================
...
# ============================================================
# تسجيل رسالة العميل الأصلية كطلب
# ============================================================

async def register_trip_message(message):
...
# ============================================================
# التأكد أن الرسالة الأصلية طلب عميل
# ============================================================
    ...
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "📩 تواصل مع العميل",
                callback_data=(
                    f"contact_customer:{trip_id}:{driver.id}"
                ),
            ),
            InlineKeyboardButton(
                "🚕 تواصل مع الكابتن",
                callback_data=(
                    f"contact_driver:{trip_id}:{driver.id}"
                ),
            ),
        ]
    ])

    driver_name = html(driver.full_name)
...
# ============================================================
# 🚕 READY — الكابتن يرد على رسالة العميل
# ============================================================

async def handle_ready_reply(update, context):
    ...
    trip_id = original_message.message_id
    ...
    if not trip:
        ...
        if not looks_like_trip(original_text):
        ...
        customer = original_message.from_user
        ...
        if not is_customer(customer.id):
    ...
    # ========================================================
    # الكابتن نفسه يبقى مسجلًا في قاعدة البيانات
    # ========================================================

    save_user(driver)

    # إذا لم يكن مسجلًا ككابتن لأي سبب، نسجله الآن.
    # هذا يمنع ظهور رسالة "اضغط أنا كابتن" مرة أخرى.
...
async def contact_customer_button(
    ...
    if customer_username:
        ...
        username = customer_username.lstrip("@")
...
def is_google_maps_link(link):
    ...
    normalized = _normalize_link(clean)
    ...
    for domain in allowed_google_maps:
...
async def message_handler(
    ...
    if is_driver_ready(text):
        ...
        if not message.reply_to_message:
            ...
            # ------------------------------------------------
            # إذا كان مسجلًا ككابتن، لا نطلب التسجيل.
            # ------------------------------------------------
    ...
    if looks_like_trip(text):
        ...
        # ----------------------------------------------------
        # إذا كان الشخص كابتن مسجل:
        # لا نحوله تلقائيًا إلى عميل.
        #
        # لكن إذا كتب طلبًا واضحًا جدًا، يبقى النظام يسمح
        # بتسجيله كطلب مثل السابق.
        # ----------------------------------------------------
...
def main():
    ...
    print(
        "💰 Prices are allowed in group messages",
        flush=True,
    )
    ...
    print(
        "🚕 Registered drivers are remembered by user ID",
        flush=True,
    )
